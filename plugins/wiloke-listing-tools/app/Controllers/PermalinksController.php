...
class PermalinksController extends Controller
{
    ...
    {
        add_filter('wilcity/filter/register-post-types/listings', [$this, 'filterRewriteRule']);
//        add_filter('wilcity/filter/register-post-types/event', [$this, 'filterRewriteRule']);
        add_filter('post_type_link', [$this, 'modifyLink'], 1, 2);
        add_filter('rewrite_rules_array', [$this, 'addRewriteRulesArray']);
    }
        ...
        if (is_object($post) && in_array($post->post_type, $aPostTypes)) {
            if (strpos($aThemeOptions['listing_permalink_settings'], '%listingLocation%') !== false) {
                ...
                if (!empty($aTerms) && !is_wp_error($aTerms)) {
                    ...
                    if ($oTerm->parent !== 0 && $aThemeOptions['taxonomy_add_parent_to_permalinks'] == 'enable' &&
