...
class NotificationsController extends Controller
{
    ...
    public function postChanged($postID, $oPostAfter, $oPostBefore)
    {
        ...
        NotificationsModel::add($oPostAfter->post_author, 'listing_status_changed', $postID,
    ...
    public function sentAddedNewListingToTheRestFollowers($oAfterPost, $listSent)
    {
        ...
        foreach ($aFollowers as $order => $aFollow) {
            ...
            if (User::userIDExists($aFollow['followerID'])) {
                NotificationsModel::add($aFollow['followerID'], 'published_new_listing', $oAfterPost->ID,
        ...
        do_action('wilcity/wilcity-mobile-app/notifications/send-published-post-to-followers', $oAfterPost,
            $aFollowers);
        wp_schedule_single_event(time() + 120, 'wilcity_start_send_notification_to_followers',
            [$oAfterPost, json_encode($aListSent)]);
    }
    ...
    public function getDisputePayment($oInfo)
    {
        $oSuperAdmin = User::getFirstSuperAdmin();
        $msg = str_replace([
            '%paymentID%',
