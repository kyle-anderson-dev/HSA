...
class SchemaController extends Controller
{
    ...
    protected function generalSchemaMarkup($post)
    {
        ...
        if (strpos($schemaSettings, 'priceRange') !== false) {
            $aPriceRange = GetSettings::getPriceRange($post->ID, false);
            if (!$aPriceRange) {
                $priceRange = null;
            } else {
            ...
            if (strpos($schemaSettings, '{{reviewDetails}}') !== false) {
                ...
                if (isset($aContentsOrder['reviews'])) {
                    $maxReviewsPerPage = isset($aContentsOrder['reviews']['maximumItemsOnHome']) ?
                        abs($aContentsOrder['reviews']['maximumItemsOnHome']) : 0;
                }
                $aReviewDetails
                    ...
                    if ($aReviewDetails) {
                        while ($aReviewDetails->have_posts()) {
                            ...
                            $aReviewsSchema[] = [
                                ...
                                'description'   => get_the_excerpt($aReviewDetails->post->ID),
        ...
        if (strpos($schemaSettings, '{{openingHours}}') !== false) {
            ...
            switch ($hourMode) {
                ...
                case 'open_for_selected_hours':
                    ...
                    $schemaSettings
                        = str_replace('"{{openingHours}}"', json_encode($aBusinessHoursSchema), $schemaSettings);
                    break;
                default:
        ...
        $schemaSettings = str_replace(
            ...
            [
                $post->post_title,
                $featuredImg,
                \Wiloke::contentLimit(apply_filters('wilcity/schema_markup/post_excerpt', 100), $post, true,
