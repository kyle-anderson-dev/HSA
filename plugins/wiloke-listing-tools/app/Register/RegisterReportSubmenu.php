	...
	public function enqueueScripts($hook){
		...
		wp_localize_script('wiloke-report-script', 'WILOKE_REPORT',
			array(
				...
				'toggle'    => empty(GetSettings::getOptions('toggle_report', false, true)) ? 'disable' : GetSettings::getOptions('toggle_report', false, true),
				'description'    => empty(GetSettings::getOptions('report_description', false, true)) ? 'If you think this content inappropriate and should be removed from our website, please let us know by submitting your reason at the form below.' : GetSettings::getOptions('report_description'),
				'thankyou' => empty(GetSettings::getOptions('report_thankyou', false, true)) ? 'Thank for reporting the issue. We value your feedback. We will try to deal with your issue as quickly as possible' : GetSettings::getOptions('report_thankyou')
			)
		);
	}
